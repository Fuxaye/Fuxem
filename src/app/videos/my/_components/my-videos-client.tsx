  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 pb-8 pt-8 text-stone-100 sm:px-6 lg:px-8">
      ...
      <section className="mt-6 rounded-3xl border border-white/10 bg-black/30 p-5 backdrop-blur-xl sm:p-6">
        <h2 className="text-lg font-semibold text-stone-100">Post Video</h2>
        ...
        <form className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2" onSubmit={handleCreate}>
          ...
          <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-stone-200 md:col-span-2">
            <input
              type="checkbox"
            ...
            />
            <span>I certify this photo/video belongs to me and I have rights to publish it (required for Public).</span>
          </label>
          ...
          {isPublic && !ownershipCertified ? (
            <p className="text-xs text-amber-200 md:col-span-2">
              Certification is required to publish publicly.
            </p>
          ) : null}
      ...
      {isLoading ? (
        ...
        <p className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-5 text-sm text-stone-300">You have not posted any videos yet.</p>
      ) : (
            ...
            <article key={video.id} className="overflow-hidden rounded-3xl border border-white/10 bg-black/30 backdrop-blur-xl">
              ...
              <div className="p-4">
                ...
                {video.isPublic && video.publicAliasPath ? (
                  <p className="mt-2 rounded-lg border border-cyan-300/25 bg-cyan-500/10 px-2.5 py-1.5 text-[11px] uppercase tracking-[0.12em] text-cyan-100">
                    Alias: {video.publicAliasPath}
                  </p>
                ) : null}
